...
contract CommissionEscrow is ReentrancyGuard, AccessControl {
    ...
    event CommissionCreated(uint256 indexed commissionId, address indexed collector, address indexed artisan, uint256 amount, uint256 deadline);
    ...
    function createCommission(address _artisan, uint256 _deadline, address _arbiter) external payable returns (uint256) {
    ...
    function releaseFunds(uint256 _commissionId) external nonReentrant {
        Commission storage commission = commissions[_commissionId];
        require(commission.status != CommissionStatus.Paid && commission.status != CommissionStatus.Refunded, "Already paid or refunded");
    ...
    function dispute(uint256 _commissionId) external {
        Commission storage commission = commissions[_commissionId];
        require(commission.status == CommissionStatus.Active || commission.status == CommissionStatus.Delivered, "Cannot dispute");
